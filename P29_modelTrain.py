...
for i in range(epoch):
    ...
    # 训练步骤开始
    # P29 训练的一些细节，把网络设置为train，这并不是说必须把网络设置为训练模式才能开始训练，之前也没有设置，但是依然可以训练
    tudui.train()
    ...
    # 所以就可以在里面写测试步骤
    # 测试步骤开始
    # P29 训练的一些细节，把网络设置为eval，这个也不是说必须把网络设置为eval，才能开始测试，之前也是没写的，但是依然可以运行
    tudui.eval()
    total_test_loss = 0
    ...
    # torch.save(tudui.state_dict(), "tudui_{}.pth".format(i)) 方式二保存模型(官方推荐)
    print("模型已保存")
    # 这个我们大概讲解完了
...
# 训练模型的套路中有一些细节需要注意
# 在训练步骤开始前，往往会有一步把网络设置为train
# 也有可能会看到，在测试网络之前，会把网络设置为eval
# 之前没有写，但是我们依然可以运行，他们的作用相对来说算是比较小的
# 他们的作用可以在官方文档上的torch.nn.Module中查看
# 把网络设置为train仅对网络中的一部分层有作用，例如说Dropout层、BatchNorm层
# 把网络设置为eval同理
# 以后在写一些代码的时候要关注当中的网络层是否有这些特殊的层，如果有的话记得一定要调用，如果没有的话调用也是没有问题的
# 加上这两个后像这样的话才是一个完整的模板
...
# 然后我们会设置训练的轮数
# 设置网络进行训练状态
# 从训练的dataloader中不断地取数据，算出来一个误差，放到优化器中进行优化
# 然后采用某种特定方式展示输出
# 一轮结束后或在特定的步数后会进行测试
# 在测试的过程中要先将模型设置为eval模式，并使用with torch.no_grad()进行设置，使网络模型中的梯度失效
# 然后从测试数据集中取数据，并计算loss
# 然后还可以构建一些特殊的指标并把它显示出来
# 最后还可以通过一些方式来展示训练的网络在测试集上的效果
# 最后可以在特定的步数或某一轮保存这个模型
# 在这里保存模型的方式是方式一，并不是方式二，方式二的保存方式其实是官方推荐的
# 一个完整的模型训练套路基本上到这里结束了，接下来会讲解如何使用GPU进行训练