...
# VGG16中有个很有意思的一点，可以用来理解卷积核是如何工作的，VGG16的输入图像是224 * 224 * 3的，经过第一个卷积层以后图像是224 * 224 * 64的，需要一个什么样的卷积核才能把3变成64呢，此处应该使用x * y * 3 * 64的卷积核(这里的x和y表示的是未知数的意思，因为笔者在写这段的时候也不知道VGG16的卷积核是什么形状的)，卷积核三个一组，分别对三个通道卷积后叠加起来，进一步需要64组卷积核来输出64个channel，其实可以简单理解为输出通道数等于卷积核个数，输入通道数等于卷积核通道数
